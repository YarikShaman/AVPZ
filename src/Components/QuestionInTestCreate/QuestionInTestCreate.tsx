...
function QuestionInTestCreate(info:{removeQuestion: (arg0: number) => void; index:number,changeOption: (arg0:Option[])=>void}) {
    let nav = useNavigate();
        ...
        <div className={"questionCreationDiv"}>
            ...
            <div className={"optionsDiv"}>
                {data.map((option,index) => (
                    <OptionInTestCreate onDataChange={handleOptionDataChange} key={index} index={index} onRemove={() => handleRemoveElement(index)} type={type} />
                ))}
            </div>
