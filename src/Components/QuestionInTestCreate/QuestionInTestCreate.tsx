...
function QuestionInTestCreate(info:{removeQuestion: (arg0: number) => void;isLast:boolean; index:number,changeOption: (arg0:Question)=>void}) {
    ...
    return (
        <div className={"questionCreationDiv"}>
            ...
            <input value={questionName} onChange={(e)=>setQuestionName(e.target.value)} placeholder={"Type Your Question"} className={"questionName nameInputs"}></input>
            ...
            <div className={"optionsDiv"}>
                {data.map((option) => (
                    <OptionInTestCreate onDataChange={handleOptionDataChange} isLast={data.length-1==option.index} key={option.index} index={option.index} onRemove={() => handleRemoveElement()} type={type} />
                ))}
            </div>
